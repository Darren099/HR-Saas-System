...
if (isProd) {
  ...
  cdn = {
    css: [
      'https://unpkg.com/element-ui@2.13.2/lib/theme-chalk/index.css' // 提前引入elementUI样式
    ], // 放置css文件目录
    js: [
...
module.exports = {
  ...
  configureWebpack: {
    ...
    // 要排除的包的名字
    // key(是要排除的包名): value(实际上是引入的包的全局的变量名)
    // 是因为要排除elementui，所以后面要引入CDN文件，CDN文件中有elementui的全局变量名
    // 首先会排除掉 定义的包名，空出来的位置会用变量来替换
    externals: externals
  },
  chainWebpack(config) {
    ...
    // 注入CDN
    // 这行代码会在打包时执行会将cdn变量注入到heml的模板中
    config.plugin('html').tap((args) => {
      // args是注入html模板的一个变量
      args[0].cdn = cdn // cdn是定义的变量
      return args
    })
