...
export default {
    ...
    return {
      ...
      rules: {
        name: [{ required: true, message: '部门名称不能为空', trigger: 'blur' },
          { min: 1, max: 50, message: '部门名称1-50个字符之间', trigger: 'blur' },
        ...
        introduce: [{ required: true, message: '部门介绍不能为空', trigger: 'blur' },
          { trigger: 'blur', min: 1, max: 300, message: '部门介绍在1-50个字符之间' }]
      }
    }
  },
  computed: {
    // 使用计算属性动态添加新增和编辑时dialog的标题
    showTitle() {
  ...
  methods: {
    ...
    async  getDepartDetail(id) {
      ...
      // 我们用父组件调用子组件的方法，先设置node，然后直接调用方法。props传值是异步的,要传过来id
    },
    // 点击确定时触发
    btnOK() {
      this.$refs.deptForm.validate(async isOK => {
        if (isOK) {
          ...
          if (this.formData.id) {
            ...
            // 新增模式
            await addDepartments({ ...this.formData, pid: this.treeNode.id }) // 调用新增接口 添加父部门的id添加pid
          }
          // 传给父组件
          this.$emit('addDepts')
          // 关闭dialog事件子组件传值父组件
          this.$emit('update:showDialog', false) // 触发事件
          // 关闭dialog时会触发el-dialog的close事件，这里不再需要重置数据
        }
      })
    },
    btnCancel() {
